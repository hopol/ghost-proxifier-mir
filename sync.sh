#!/usr/bin/env bash
# 手动镜像 liliBestCoder/ghost-proxifier-pro 的 master 分支。
set -euo pipefail

UPSTREAM_URL="https://github.com/liliBestCoder/ghost-proxifier-pro.git"
UPSTREAM_WEB_URL="https://github.com/liliBestCoder/ghost-proxifier-pro"
UPSTREAM_BRANCH="master"
MIRROR_REPO="https://github.com/hopol/ghost-proxifier-mir"
...
git add upstream
git commit -m "chore: mirror ghost-proxifier-mir source $SOURCE_VALUE ($SHORT_COMMIT)"
git tag -a "mirror-source-$SHORT_COMMIT" -m "Mirror ghost-proxifier-mir source $SOURCE_VALUE ($SHORT_COMMIT)"
git push origin HEAD
git push origin "mirror-source-$SHORT_COMMIT"
echo "已同步 ghost-proxifier-mir $UPSTREAM_BRANCH：$UPSTREAM_COMMIT"
